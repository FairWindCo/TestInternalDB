...
<%-- JCombobox --%>
<%@include file="/WEB-INF/include/tokenizer_include.jsp" %>
<html>
<head>
    <title>Test Multi Select input</title>
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $('#subdivisions-input').selectivity({
        ajax: {
      ...
      $('#tokenize2').tokenize({
        datas: "${pageContext.request.contextPath}/category/subdivs",
        valueField:"Value",
...
<h1>Custom Build Demo's</h1>
<h2>Select a GitHub repository</h2>
<p>
<select id="subdivisions-input" class="selectivity-input" name="traditional[multiple]" multiple>
</select>
</p>
<p>Demo use-cases showing how a custom build of selectivity with a custom build is behaving.</p>
...
<ul class="dev-instructions">
  ...
  <li>Then open <a href="http://localhost:3000/demos/custom.html">http://localhost:3000/demos/custom.html</a>
...
<p class="relative"><span id="single-input-with-submenus" class="selectivity-input"></span></p>
<h2>Select multiple cities</h2>
<p class="relative"><span id="multiple-input" class="selectivity-input"></span></p>
...
<select id="single-select-box" class="selectivity-input" data-placeholder="No city selected" name="traditional[single]">
